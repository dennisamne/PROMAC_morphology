...
The analysis revealed 165 genes more abundant in the TN-treatment, and 178 more abundant in the C-treatment (@tbl-overview). As you soon will see, the amount of genes in the target gene list were so few, that they were included in the large scale analysis instead, but are still presented here for you to see. The section summarising the expressino results is the section called "The rest of the DE-genes".
...
edgeR_KO %>%
  ...
  mutate(process = case_when(
    gene %in% aggregationGenes ~ "Aggregation",
    gene %in% LPSOMV ~ "OMVs",
  ...
  theme_minimal()  +
  scale_shape_manual('Process', values = c(0,1,2,5))

ggsave("../results/manualCats.png", bg = "white")
```
...
The TN-abundant genes did not have a clear categorical response, but was spread out across categories. Notable differences are the occurrence of Archaea and Flacobacteriia in the C-abundant and TN-abundant fractions respectively.
...
#Creating vectors for up and downregulated orfs.
...
bbmap_p %>%
  ...
  summarise(tpm = mean(tpm)) %>% # summarising mean tpm per treatment
  ungroup() %>%
  mutate(type = case_when(orf %in% upReg ~ "Up", #Mutating in up/downregulation information
                          orf %in% downReg ~ "Down",
                          TRUE ~ "non-DE")) %>%
...
ggsave("../results/COG_cats.tiff", bg = "white", width = 18, height = 12, unit = "cm")
...
# Adding taxonomy to plot in each category and DE-fraction
bbmap_p %>%
  inner_join(sample_ID %>% filter(day == "17"), by = "sample") %>% # Selecting relevant day
  filter(treatment %in% c("TN","C")) %>% # Selecting relevant treatments
  ...
  inner_join(taxonomy %>% filter(superkingdom %in% c("Bacteria", "Archaea")), by = "chr") %>%
  mutate(class = na_if(class, "no support"),
         phylum = na_if(phylum, "no support")) %>% #Repacing the no support wth NA
  mutate(taxa_group = coalesce(class, phylum, superkingdom))  %>%
...
ggsave("../results/COG_cats_taxpnomy.png", bg = "white")
```
