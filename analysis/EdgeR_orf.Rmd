...
# Genes related to OMV's on the other hand GO:0061701 exist.
...
edgeR_orfs %>%
  ...
  filter(GOs %in% c("GO:0032060","GO:0061701"))
```
There are no GO-terms related for blebbing or OMV's in the up/downregulated genes (GO-terms GO:0032060,GO:0061701).
...
```
### Downregulated genes
Out of the downregulated genes, only `r edgeR_orfs %>% filter(type == "Down") %>% inner_join(eggnogs %>% distinct(orf,Preferred_name),by = "orf") %>% filter(Preferred_name != "-") %>% nrow()` are annotated.
```{r}
kable(
  edgeR_orfs %>%
  filter(type == "Down") %>%
  ...
  scroll_box(width = "100%", height = "500px")
```
### Archaea of my interest
Out of the `r edgeR_orfs %>% filter(type == "Down") %>% inner_join(taxonomy, by = "orf") %>% filter(domain == "Archaea") %>% nrow()` archaeal genes, only `r edgeR_orfs %>% filter(type == "Down") %>% inner_join(taxonomy, by = "orf") %>% filter(domain == "Archaea") %>% inner_join(eggnogs %>% distinct(orf, Preferred_name, best_og_desc),by = "orf") %>% filter(Preferred_name != "-")` were annotated. Potentially we are missing important genes here.
```{r}
kable(
  edgeR_orfs %>%
  filter(type == "Down") %>%
